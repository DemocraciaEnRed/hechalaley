...
test('MemoryCache: adding an item with tags', async (t) => {
  ...
  await cache.set('one-two', 123123, ['one', 'two'])
...
test('MemoryCache: deleting multiple items by tag', async (t) => {
  ...
  await cache.set('one-two', 121212, ['one', 'two'])
  await cache.set('two-one', 121212, ['one', 'two'])
  await cache.set('one-three', 13131313, ['one', 'three'])
