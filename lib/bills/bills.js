...
function showPolitician (id) {
  request.get('/api/politicians/' + id + '?populate=[\'jurisdiction\']')
...
function getBill (ctx, next) {
  request.get('/api/bills/' + ctx.params.id + '?populate.coSigners=1&published=true')
    .end(function (err, res) {
...
function getStages (ctx, next) {
  request.get('/api/bills/' + ctx.params.id + '/stages?published=true')
    .end(function (err, res) {
...
function getLastContents (ctx, next) {
  ...
  request.get('/api/stages/' + lastStage._id + '?markdown=true&populate.authors=1&published=true')
    .end(function (err, res) {
      if (err) throw err
...
function getAuthor (ctx, next) {
  request.get('/api/politicians/' + ctx.bill.author + '?populate=[\'jurisdiction\']')
    .end(function (err, res) {
      if (err) throw err
